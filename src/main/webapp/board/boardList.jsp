			...
			<div class="row featurette">
				<div>
					...
					*/
					...
					int currentPage = 1;
					if (tempPage != null) { // 사용자가 pagination을 클릭하여 정상적인 값이 들어갔을 때
						try {
					...
					// 6.시작번호와 끝번호 사이에 해당하는 게시글을 조회
					List<BoardDTO> boardList = bs.searchBoardList(rDTO);

					// 글 제목이 20글자 이상이면 19까지만 보여주고 나머지는 ...으로 처리
					...
					<br>
					<%-- 					
					<br> 총 게시글의 수 : ${ totalCount } 
					<br> 한 화면에 보여줄 게시글의 수 : ${ pageScale }건 
					<br> 총 페이지 수 : ${ totalPage }장 
					<br> 현재 페이지 pagination 번호 : ${ currentPage }
					<br> 시작 번호 : ${ startNum }
					<br> 끝 번호 : ${ endNum } 
					--%>
					총
					...
					<div id="boardList" style="min-height: 600px">
						<input type="button" value="글쓰기" class="btn btn-success btn-sm"
							id="btnWrite" style="margin-top: 50px">
						<table class="table table-hover">
							...
							<tbody>
								...
								<c:forEach var="bDTO" items="${ boardList }" varStatus="i">
									<tr>
										<td><c:out
												value="${ totalCount - (currentPage-1) * pageScale - i.index }"></c:out></td>
										...
										<td><fmt:formatDate pattern="yyyy-MM-dd a HH:mm"
												value="${ bDTO.input_date }"></fmt:formatDate></td>
									</tr>
								</c:forEach>
							</tbody>
						</table>
					...
					<div id="boardSearchDiv" style="text-align: center;">
						<form action="boardList.jsp" id="boardSearchFrm">
							<select name="field" id="field" style="height: 30px">
								...
								name="currentPage" value="${ tempPage }" style="height: 30px">
