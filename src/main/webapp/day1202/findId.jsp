		...
		<div id="container">
			<div id="searchFrm">
				<form name="popupFrm" action="findId.jsp" method="get">
					<input type="text" name="id" id="id" value="${ param.id }"
						style="width: 200px" /> <input type="text" style="display: none;">
			...
			<c:if test="${ not empty param.id }">
				<div id="searchResult">
					...
					%>
					<c:set var="resultCss" value="fail"></c:set>
					<c:set var="resultMsg" value="사용 불가능"></c:set>
					<c:if test="${ flagId }">
						<c:set var="resultCss" value="success"></c:set>
						<c:set var="resultMsg" value="사용 가능"></c:set>
					</c:if>
					입력하신 아이디 [<strong>${ param.id }</strong>]는 <span
						class="${ resultCss }"><c:out value="${ resultMsg }"></c:out></span>
					<c:if test="${ flagId }">
