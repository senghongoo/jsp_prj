...
<link rel="shortcut icon"
	href="http://192.168.10.88/jsp_prj/common/images/favicon.ico">
...
<script type="text/javascript">
	$(function() {
		...
		$("#btnFileUpload").click(function() {
			...
			// ajax로 파일 업로드
			var form = $("#frm")[0]; // 파라미터 전송 방식
			var formData = new FormData(form); // 파라미터 전송 방식 -> binary 전송 방식
				...
				<div class="col-md-7">
					...
					<form action="ajaxUploadFrmProcess.jsp" method="post" id="frm">
						<label>업로더</label> <input type="text" name="uploader"
							id="uploader"><br /> <label>대상 연령</label>
						<c:forEach var="age" begin="10" end="80" step="10">
							<input type="checkbox" name="age" value="${ age }">
							<c:out value="${ age }대" />
						</c:forEach>
						<br> <label>파일</label><br> <img
							src="../upload/default_img.png" id="preview"
							style="width: 100px; height: 100px; border-radius: 50px;">
						<input type="button" value="파일선택" id="btnFile"
							class="btn btn-primary btn-sm" /><br> <input type="button"
							value="파일업로드" id="btnFileUpload" class="btn btn-info btn-sm" /><br>
						...
						<input type="file" accept="image/*" name="upfile" id="upfile"
							style="display: none" /><br> <input type="hidden"
							name="profile" id="profile" style="display: none" /><br> <input
							type="button" value="정보업로드" class="btn btn-success btn-sm"
							id="btnUpload" /><br>
					</form>
				</div>
			</div>
