		...
		<div class="container marketing">
			...
			<div class="row featurette">
				<div class="col-md-7">
					...
					// <jsp:param> 태그에 들어간 값을 request.getParameter로 받는다.
					String name=request.getParameter("name");
					String age=request.getParameter("age");
					%>
					<label><%=name %>(<%=age %>)님이 다음 중 가장 좋아하는 캐릭터는?</label><br>
						<%
						for (int i = 0; i < names.size(); i++) {
						%>
						<%=i + 1%>.
						<input type="checkbox" name="character" value="<%=names.get(i)%>"><%=names.get(i)%>&nbsp;
						<%
						} // end for
						%>
				</div>
