...
public class WebMemberDAO {
	...
	public boolean selectId(String id) throws SQLException {
		...
		try {
			...
			con = dbCon.getConn();
			// 4.쿼리문 생성 객체 얻기
			String selected = "select id from web_member where id=?";
			pstmt = con.prepareStatement(selected);
			// 5.바인드 변수에 값 설정
			pstmt.setString(1, id);
			// 6.쿼리문 수행 후 결과 얻기
			rs = pstmt.executeQuery();

			resultFlag = !rs.next(); // 아이디가 존재하면 true - 사용 불가
		} finally {
			// 7.연결 끊기
	...
	public ParamDTO selectMember(String id) throws SQLException {
		...
		try {
			...
			selectMember.append("	select name, email, birth, input_date, ip, intro, profile, loc	")
